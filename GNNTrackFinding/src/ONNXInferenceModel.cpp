...
ONNXInferenceModel::ONNXInferenceModel(const std::string& name, OrtLoggingLevel logLevel, bool useCuda,
                                       std::size_t cudaDeviceIndex)
    : m_env(std::make_unique<Ort::Env>(logLevel, name.c_str())),
  ...
  if (useCuda) {
    // Prefer CUDA but fall back to CPU for any op the CUDA EP does not support.
    OrtCUDAProviderOptions cudaOptions{};
    cudaOptions.device_id = static_cast<int>(cudaDeviceIndex);
...
std::vector<Ort::Value> ONNXInferenceModel::runInference(const std::vector<float>& inputData,
                                                         const std::vector<int64_t>& inputShape, int cudaDeviceIndex) {
  if (!m_modelLoaded) {
