...
struct CaloExtrapMonitor {
  ...
  std::string summary() const {
    ...
    std::string text =
        fmt::format("Calorimeter-face extrapolation summary: {} attempts, {} reached the face, {} failed "
                    "({:.2f}%: {} not reached, {} propagation errors); {} tracks had no measured smoothed start state.",
                    a, o, failed, 100.0 * failRate, nr, pf, ns);
    if (b > 0) {
      text += fmt::format(
          " {} reached the barrel face, of which {} also reached an endcap disc (second AtCalorimeter state).", b, b2e);
    }
    return text;
  }
};
...
public:
  ...
  CaloStateAppender(const IActsGeoSvc& geo, const Acts::GeometryContext& geoCtx,
      ...
      : m_geo(geo), m_geoCtx(geoCtx), m_magCtx(magCtx), m_maxSteps(cfg.maxSteps), m_addEndcapState(cfg.addEndcapState) {
  ...
  void addCaloState(const Alg& alg, const TrackProxy& trackTip, edm4hep::MutableTrack& track,
    ...
    const auto caloResult = ACTSTracking::extrapolateToCaloFace(caloPropagator, *startParams, m_geo.caloSurfaceGeoIds(),
                                                                m_geoCtx, m_magCtx, m_maxSteps);
