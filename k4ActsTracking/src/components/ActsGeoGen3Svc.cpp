...
StatusCode ActsGeoGen3Svc::initialize() {
  ...
  auto innerTrackerBarrelElem = builder.findDetElementByName("InnerTrackerBarrel").value();
  auto innerTrackerBarrel     = builder.addLayers(innerTrackerBarrelElem, "XYZ", AxisR, std::regex{"layer\\d_\\d"});
