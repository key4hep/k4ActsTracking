...
void addCylindricalBeampipe(ContainerBlueprintNode& node, bool designateMaterial = false, double rMax = 10_mm,
  ...
  if (designateMaterial) {
    beampipe = withMaterial(std::move(beampipe), "Beampipe_Material", Acts::CylinderVolumeBounds::Face::OuterCylinder);
  }
...
void addCylindricalSolenoid(ContainerBlueprintNode& node, double rMin, bool designateMaterial = false) {
  std::shared_ptr<BlueprintNode> solenoid =
      std::make_shared<Acts::StaticBlueprintNode>(std::make_unique<Acts::TrackingVolume>(
          ...
          "Solenoid"));
  if (designateMaterial) {
    solenoid = withMaterial(std::move(solenoid), "Solenoid_Material", Acts::CylinderVolumeBounds::Face::OuterCylinder);
  }
...
void addUngroupedEndcapSide(
    ...
    const std::regex& filter, AxisDefinition axes, const std::string& labelPrefix, const Acts::ExtentEnvelope& envelope,
...
void addBothEndcapSides(ActsPlugins::DD4hep::BlueprintBuilder& builder, Acts::BlueprintNode& parent,
  ...
  if (layout == Layout::Grouped) {
    ...
    addUngroupedEndcapSide(builder, parent, container, posFilter, axes, posLabel, envelope, keyXform, std::move(onPos));
    addUngroupedEndcapSide(builder, parent, container, negFilter, axes, negLabel, envelope, keyXform, std::move(onNeg));
  }
}
...
/// @returns The barrel blueprint node
template <typename MatchTransformF = decltype(identityKey)>
std::shared_ptr<ContainerBlueprintNode>
makeBarrel(ActsPlugins::DD4hep::BlueprintBuilder& builder, const TrackerSpec& spec,
           const Acts::ExtentEnvelope& envelope = kBarrelEnvelope, MatchTransformF keyXform = identityKey,
           const std::optional<LayerMaterial>& material = std::nullopt) {
  ...
  return builder.layersFromSensors()
      ...
      .onLayer(
          designateLayers(material, Acts::CylinderVolumeBounds::Face::InnerCylinder, spec.barrelContainer, unsetXYCoG))
      .build();
}
...
makeRegularTracker(ActsPlugins::DD4hep::BlueprintBuilder& builder, const TrackerSpec& spec,
                   const std::string& trackerName, const std::optional<LayerMaterial>& barrelMaterial = std::nullopt,
...
std::shared_ptr<CylinderContainerBlueprintNode>
makeNestedInnerTracker(ActsPlugins::DD4hep::BlueprintBuilder& builder,
                       std::shared_ptr<CylinderContainerBlueprintNode>&& vertex,
  ...
  addBothEndcapSides(builder, *innerTracker, spec.endcapContainer, spec.endcapPosOuterFilter, spec.endcapNegOuterFilter,
                     spec.endcapAxes, spec.layout, kTrackerEnvelope, fmt::format("{}|layer_pos", spec.endcapContainer),
                     fmt::format("{}|layer_neg", spec.endcapContainer), identityKey, endcapMaterial);
...
void addCaloBarrel(BlueprintNode& parent, const IActsGeoSvc::CaloFaceSurfaces& calo) {
  constexpr double pad = 1_mm;
  auto bounds = std::make_shared<Acts::CylinderVolumeBounds>(std::max(0.0, calo.barrelRMin - pad), caloVolumeRMax(calo),
