 ...
 */
  inline bool track_duplicate_compare(const edm4hep::Track& trk1, const edm4hep::Track& trk2) {
    ...
    // Copy to mutable handles: edm4hep's Track::getTrackState is not (yet) const.
    edm4hep::Track t1 = trk1;
    edm4hep::Track t2 = trk2;
    const auto     ts1        = t1.getTrackState(edm4hep::TrackState::AtIP);
    const auto     ts2        = t2.getTrackState(edm4hep::TrackState::AtIP);
...
ACTSDuplicateRemoval::ACTSDuplicateRemoval(const std::string& name, ISvcLocator* svcLoc)
    : Transformer(name, svcLoc, KeyValues("InputTrackCollectionName", {"TruthTracks"}),
